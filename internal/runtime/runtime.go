...
func (r *Runtime) EnsureMaterialized() error {
	...
	// Always-overwrite files — these are source-of-truth assets shipped
	// with the binary. Compose + Dockerfile + php.ini must reflect the
	// current version.
	...
	// First-run-only file — the Caddyfile holds per-project site
	// blocks that are regenerated by internal/network/caddy.go, NOT
	// by this embed. Overwriting on every command wiped the sites
	// whenever a CLI command ran before an `applyCaddySites` call.
	caddyPath := filepath.Join(r.DataDir, "frankenphp", "Caddyfile")
	if _, err := os.Stat(caddyPath); os.IsNotExist(err) {
		data, err := assets.FS.ReadFile("dockerfiles/Caddyfile")
		if err != nil {
			return fmt.Errorf("read embed Caddyfile: %w", err)
		}
		if err := os.WriteFile(caddyPath, data, 0o644); err != nil {
