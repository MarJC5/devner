...
if ! grep -q "$host" /etc/hosts; then
    ...
    sudo -- sh -c -e "echo '127.0.0.1\t${host}.local' >> /etc/hosts" # IPv4
    sudo -- sh -c -e "echo '::1\t\t${host}.local' >> /etc/hosts" # IPv6
    sudo -- sh -c -e "echo '127.0.0.1\twww.${host}.local' >> /etc/hosts" # IPv4 www
    sudo -- sh -c -e "echo '::1\t\twww.${host}.local' >> /etc/hosts" # IPv6 www
    echo -e "${GREEN}$host${NC} added to /etc/hosts"
else
...
EOF
    ...
    echo -e "Virtual host for ${GREEN}$host.local${NC} created and enabled."
else
    echo -e "Virtual host file for ${YELLOW}$host.local${NC} already exists."
fi

if [ -f "$NGINX_SITES_CONFIG" ]; then
    escaped_host=$(echo "$host.local" | sed 's/\./\\./g')
    ...
    if grep -q "$mapping" "$NGINX_SITES_CONFIG"; then
        echo -e "Mapping for ${YELLOW}$host.local${NC} already exists in the Nginx configuration."
    else
        ...
        echo -e "Mapping for ${GREEN}$host.local${NC} added to the Nginx configuration."
    fi
fi
